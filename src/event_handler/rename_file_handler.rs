...
use crate::event_handler::{Processor, format_filename_with_timestamp, list_files};
...
impl Processor for RenameFileHandler {
...
mod tests {
    ...
    fn test_handle_file_event_no_duplicate() {
        ...
        // 执行处理事件
        handler.proceed(&file_path.clone().into()).unwrap();
